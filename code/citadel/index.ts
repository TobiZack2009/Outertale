...
export async function sad () {
   ...
   await Promise.all([ epilogueLoader, renderer.pause(1000) ]);
...
export async function armaloop () {
   const arma = music.approach.instance(renderer, { offset: Math.min(SAVE.data.n.armaloop, 230.4) });
   if (game.room[0] !== 'c') {
      arma.gain.value = 0;
      arma.rate.value = 0;
   }
...
export function archiveBattle (group: OutertaleGroup, dest: string, face: CosmosDirection) {
   const handler = async () => {
      ...
      game.music = music.archive.instance(renderer, { offset: archiver.offset });
...
export const areaC = {
   ...
   scripts: {
      ...
      async final_frontier () {
         ...
         await Promise.all([ part1loader, renderer.pause(4000) ]);
         ...
         if (SAVE.data.b.svr) {
            ...
            await Promise.all([ part2loader, renderer.pause(2500) ]);
            ...
            await Promise.all([ part3loader, renderer.pause(3000) ]);
         ...
         /** SECTION 1B */
         {
            ...
            creditsDisplay.attach(
               ...
               genOpponent('finalghost', 160, y1(3), 120, SAVE.data.n.state_wastelands_dummy === 4)
            );
      ...
      async exitarchive () {
         ...
         await Promise.all([ epicLoader, renderer.pause(1000) ]);
      ...
      async final () {
         ...
         if (calcLV() > 14) {
            ...
            const cym = sounds.cymbal.instance(renderer, { offset: 2.5 });
            cym.gain.value /= 4;
         ...
         if (SAVE.data.n.state_citadel_refuse > 0) {
            ...
            await Promise.all([ twLoader1, renderer.pause(2000) ]);
         } else {
            await renderer.pause(2000);
            await cam.position.modulate(renderer, 2000, { y: -100 });
            await renderer.pause(3000);
            ...
            await Promise.all([ twLoader1, renderer.pause(3000) ]);
         }
         ...
         if (choicer.result === 0 || world.bad_robot || world.trueKills > 29) {
            ...
            renderer.detach('menu', fd);
            game.music = music.archive.instance(renderer);
            game.movement = true;
         } else {
            twAssets1.unload();
   ...
   teleports: {
      ...
      async c_elevator1 () {
         ...
         if (SAVE.data.n.plot === 72 && SAVE.data.n.plot_epilogue < 4 && !world.runaway) {
            ...
            if (!bad) {
               ...
               await renderer.pause(650);
               goatdad.face = 'left';
               await Promise.all([ storyLoader, renderer.pause(1250) ]);
      ...
      async c_courtroom (roomState) {
         ...
         if (!world.genocide && SAVE.data.n.plot < 71) {
            ...
            if (ut_geno) {
               ...
               await Promise.all([ choiceLoader, renderer.pause(10000) ]);
      ...
      async c_bastion (roomState) {
         ...
         if (world.genocide && SAVE.data.n.plot < 71.2) {
            ...
            await Promise.all([ grabLoader, renderer.pause(200) ]);
   ...
   events.on('teleport').then(async () => {
      ...
      if (inArchive) {
         ...
         renderer.pause(2000).then(() => {
            SAVE.data.n.state_citadel_archive < 6 && (game.music = music.archive.instance(renderer));
            renderer.detach('menu', fd);
            world.cutscene_override = false;
            game.movement = true;
            engageDelay();
      ...
      switch (died ? 0 : SAVE.flag.n._genocide_milestone_last) {
         ...
         case 6:
            ...
            break;
         case 7:
            await dialogue('auto', ...text.a_citadel.genotext.respawn7());
            break;
      }
      game.movement = true;
   });
...
events.on('loaded', {
   priority: -1,
   async listener () {
      SAVE.isCanon() || (SAVE.data.b.oops = true);
      saver.svr = !SAVE.isCanon() || SAVE.flag.b.$svr;
