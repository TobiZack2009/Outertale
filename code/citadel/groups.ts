...
export const hyper = {
   ...
   color_reset () {
      ...
      for (const echo of battler.SOUL.metadata.cyanLeapEcho.splice(0, battler.SOUL.metadata.cyanLeapEcho.length)) {
      ...
      for (const obj of battler.SOUL.metadata.shot_obj.splice(0, battler.SOUL.metadata.shot_obj.length)) {
         if (battler.SOUL.metadata.layer === null) {
...
export const hypervisor = {
   ...
   instance ({
      ...
      const inst = daemon.instance(renderer, { offset: position });
   ...
   silence () {
      ...
      for (const inst of hypervisor.instances.splice(0, hypervisor.instances.length)) {
   ...
   tick () {
      ...
      if (hypervisor.time === phase.duration) {
         ...
         for (const object of objects.filter(object => !hyper.arena.objects.includes(object))) {
            object.tick();
         }
      ...
      if (world.postnoot || SAVE.flag.n.neutral_twinkly_stage < 2) {
         ...
         if (!world.postnoot) {
            ...
            const soundjump = (offset: number) => {
               sounds.save.stop();
               idiot?.stop();
               idiot = music.idiot.instance(renderer, { offset });
         ...
         if (world.postnoot) {
            if (rep < 7) {
               ...
               await Promise.all([ twLoader17, renderer.pause(2000) ]);
            }
         } else {
            sounds.noise.instance(renderer);
         }
      ...
      if (!world.postnoot) {
         ...
         if (SAVE.flag.n.neutral_twinkly_stage < 3) {
            ...
            await renderer.when(() => {
               if (i < 180) {
                  if (i % 2 === 0) {
                     for (const inst of repinst.splice(0, repinst.length)) {
                        inst.stop();
                     }
                     repinst.push(
                        ...repbase.map(info => {
                           const inst = info.daemon.instance(renderer, { offset: info.position });
         ...
         if (SAVE.flag.n.neutral_twinkly_stage < 4) {
            ...
            while (true) {
               ...
               hyper.color_reset();
               hypervisor.save_times.splice(0, hypervisor.save_times.length);
               hypervisor.time = 0;
               hypervisor.time_load = NaN;
               ...
               for (const file of hypervisor.files) {
                  file.instances.splice(0, file.instances.length);
                  file.time = NaN;
               }
               hypervisor.slots = 0;
               ...
               if (hy.active && hypervisor.phase === 6) {
                  ...
                  await Promise.all([ twLoader16, renderer.pause(4000) ]);
                  break;
               } else if (hy.active) {
                  hypervisor.silence();
                  hy.resolve();
                  ...
                  const safe = {
                     ...
                     mus_start () {
                        ...
                        safe.mus_daemon.instance(renderer, { offset: safe.mus_time });
                     },
                  ...
                  if (safe.result) {
                     ...
                     hypervisor.phase = hyper.phase();
                     await Promise.all([ phaseLoader, phaseLoaderAdditional, hyper.static() ]);
                  } else {
                     hypervisor.random.value = SAVE.flag.n.neutral_twinkly_base;
                     phaseAssets?.unload();
                     await hyper.static();
      ...
      init () {
         ...
         battler.resume(async () => {
            ...
            if (SAVE.data.n.hp > 0) {
               ...
               game.movement = false;
               await resetBox(true);
               battler.music = music.galactomania.instance(renderer);
               ...
               header('z2').then(() => {
                  ...
                  battler.music = music.galactomaniaQuiet.instance(renderer);
               ...
               header('z4').then(async () => {
                  ...
                  beats = 180;
                  pmapp = [ 1, 6, 6, 6, 6, 6 ];
                  battler.music = music.galactomaniaFinal.instance(renderer);
   ...
   final: new OutertaleGroup({
      ...
      init () {
         ...
         if (SAVE.flag.n.pacifist_marker < 1) {
            battler.music = music.ASGORE.instance(renderer, { offset: 0.96 });
