...
import { MadDummyMetadata, epiphany, resetBox, standardSize } from '../common/api';
               ...
               if (state.vars.flirted) {
                  ...
                  if (state.choice.act === 'turn') {
                     ...
                     music.legsIntro.instance(renderer).source.addEventListener('ended', () => {
                        battler.music = music.legs.instance(renderer, { offset: (60 / 148) * 16 });
                     });
      ...
      handler: battler.opponentHandler({
         ...
         defaultStatus: state =>
            state.vars.wires === 0
               ...
               : () => text.b_opponent_mettaton2.statusX(),
         bubble: () => [
            world.genocide
               ? { x: 180, y: 25 }
         ...
         async fight ({ volatile, choice, vars, dialogue }, truePower) {
            if (world.bad_robot) {
               ...
               if (result) {
                  ...
                  const sfx = sounds.surge.instance(renderer, { offset: 1.5 });
