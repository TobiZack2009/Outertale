   ...
   if (delimiter === '/') {
      ...
      switch (key) {
         ...
         case 'p':
            ...
            this.sounds = preset.voices[0] ?? [];
            game.text = '';
            return;
         case 'v':
            this.sounds = speech.state.preset.voices[+value] ?? [];
            return;
      }
   }
...
export const battler = {
   ...
   async monster (
      ...
      if (battler.multitext.active) {
         ...
         const typer_local = new CosmosTyper({ renderer, formatter });
...
export function calcLV () {
   ...
   return lv + (SAVE.data.b.oops || (saver.svr && !SAVE.data.b.svr) ? 1 : 0);
}
...
atlas.navigators.register({
   ...
   frontEndNameConfirm: new CosmosNavigator<string>({
      flip: true,
      grid: () => [
         values.nameChoiceRestrictions.includes(frontEnder.name.value_true.toLowerCase()) ? [ 'no' ] : [ 'no', 'yes' ]
      ],
      next (self) {
         ...
         new CosmosObject({
            ...
            objects: [
               menuText(180, 68 - 4, () => {
                  ...
                  const fontNS = values.nameChoiceFonts[frontEnder.name.value.toLowerCase()];
                  if (fontNS === void 0) {
               ...
               menuText(
                  ...
                  () =>
                     `§fill=${menuFinder('frontEndNameConfirm', 'no') ? '#ff0' : '#fff'}§${
                        values.nameChoiceRestrictions.includes(frontEnder.name.value_true.toLowerCase())
                           ? text.menu.confirm3
                           : text.general.no
                     }`
               ).on('tick', function () {
                  this.alpha.value = game.input ? 1 : 0;
                  this.offsets[0].x = values.nameChoiceRestrictions.includes(frontEnder.name.value_true.toLowerCase())
                     ? values.nameGoBackX
                     : values.nameNoX;
               }),
               menuText(
      ...
      next (self) {
         ...
         switch (selection) {
            ...
            case 'touch':
               ...
               break;
            case 'mods':
               backend !== null ? backend.exec('mods') : reload_full();
               break;
         }
      },
      prev () {
...
addEventListener('blur', () => {
   ...
   mobile.reset();
   for (const key of Object.values(keys)) {
      key.state.splice(0, key.state.length);
   }
});
