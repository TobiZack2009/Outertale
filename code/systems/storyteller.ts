...
export class CosmosAsset<A = any> {
   ...
   async load (owner: any = this) {
      ...
      const promise = (this.promise ??= this.loader().then(value => {
         this.promise === promise && (this._value = value);
...
export class CosmosInstance extends CosmosEventHost<{ stop: [] }> {
   ...
   get position () {
      if (this.source.buffer === null) {
         ...
         this.analyser.getFloatTimeDomainData(CosmosAudioUtils.samples);
         const value = CosmosAudioUtils.samples[0] / this.source.buffer.sampleRate;
         if (value === 0) {
   ...
   clear (...keys: A[]) {
      for (const key of keys) {
         ...
         if (layer.objects.length !== 0) {
            layer.dirty = true;
            for (const object of layer.objects.splice(0, layer.objects.length)) {
   ...
   tick () {
      ...
      for (const layer of this._layers) {
         if (layer.dumpster.length !== 0) {
            for (const object of layer.dumpster.splice(0, layer.dumpster.length)) {
      ...
      if (CosmosImageUtils.dumpster.length !== 0) {
         for (const texture of CosmosImageUtils.dumpster.splice(0, CosmosImageUtils.dumpster.length)) {
...
> extends CosmosBase<A> {
   ...
   set offsets (value) {
      this._offsets.value.splice(0, this._offsets.value.length);
      this._offsets.value.push(...value);
   }
   ...
   tick () {
      if (this.dumpster.length !== 0) {
         for (const object of this.dumpster.splice(0, this.dumpster.length)) {
   ...
   draw (style: CosmosStyle) {
      ...
      if (fontSize === 0 || fontFamily?.loaded !== true || this.transparent()) {
         ...
         if (update) {
            ...
            const cursor = { x: 0, y: 0 };
            const mystify = [] as string[];
            const mystify_bucket = [] as string[];
            ...
            const shake = { x: 0, y: 0 };
            const subspacing = { x: 0, y: 0 };
            const wordify = { index: -1, size: 0, word: '' };
               ...
               if (char === '\n') {
                  ...
                  switch (key) {
                     ...
                     case 'mystify':
                        ...
                        mystify_bucket.splice(0, mystify_bucket.length);
                        break;
                     case 'random':
                     ...
                     case 'reset':
                        ...
                        mystify.splice(0, mystify.length);
                        shake.x = 0;
                        shake.y = 0;
                  ...
                  if (mystify.length !== 0) {
                     ...
                     if (mystify.length === 0) {
                        mystify.push(...mystify_bucket.splice(0, mystify_bucket.length));
                     } else {
