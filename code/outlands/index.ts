...
export async function exitscene (lateleave = false, sleep = false) {
   ...
   await Promise.all([ exitfailLoader, overlay.alpha.modulate(renderer, 3000, 1).then(() => renderer.pause(1000)) ]);
      ...
      switch (subscript) {
               ...
               if (choicer.result === 1) {
                  ...
                  if (choicer.result === 1) {
                     ...
                     if (choicer.result === 1) {
                        ...
                        game.music?.stop();
                        game.music = music.home.instance(renderer);
         ...
         case 'socks': {
            ...
            if (SAVE.data.b.svr) {
               ...
               if (SAVE.isCanon() && !SAVE.data.b.oops && !SAVE.data.b.key_coffin) {
               ...
               if (world.darker) {
                  ...
                  if (SAVE.isCanon() && !SAVE.data.b.oops) {
                     coffin = true;
                  }
               } else {
            ...
            if (coffin) {
               if (choicer.result === 0) {
                  ...
                  saver.svr || (SAVE.data.b.key_coffin = true);
               } else {
         ...
         case 'chairiel': {
            ...
            if (!toriSV() && (SAVE.data.b.w_state_catnap || SAVE.data.n.plot > 17)) {
               ...
               if (choicer.result === 0) {
                  ...
                  await fd.alpha.modulate(renderer, 300, 0);
                  renderer.detach('menu', fd);
                  game.music = music.home.instance(renderer, { offset: lampState.music?.position ?? 0 });
                  game.music.gain.value = 0;
                  game.music.rate.value = world.ambiance;
                  return;
               } else {
...
export async function outlandsTPE (from: string, to: string) {
   ...
   switch (to) {
      ...
      case 'w_courtyard':
         ...
         break;
      case 'w_wonder': {
         (!SAVE.isCanon() || 7 <= SAVE.data.n.plot || SAVE.flag.b.$cocoa) && instanceDestroy([ 'w_goner' ]);
      ...
      case 'w_start': {
         if (!awakenSystem.state && SAVE.data.b.svr && SAVE.data.s.room === 'w_start') {
            ...
            if (!SAVE.data.b.oops) {
               ...
               await Promise.all([ LOADER_1, renderer.pause(2000) ]);
               ...
               await Promise.all([ LOADER_2, renderer.pause(1850) ]);
               ...
               await Promise.all([ LOADER_3, renderer.pause(1650) ]);
               ...
               await Promise.all([ LOADER_4, renderer.pause(1500) ]);
      ...
      case 'w_toriel_asriel': {
         ...
         if ((game.music || from === '_void') && SAVE.data.n.state_wastelands_toriel !== 2) {
            const mus = music.homeAlt.instance(renderer, { offset: game.music?.position ?? 0 });
            mus.gain.value = 0;
      ...
      case 'w_exit': {
         ...
         if (!roomState.goatbro && world.genocide && SAVE.data.n.plot < 16) {
            ...
            if (SAVE.data.n.plot < 15) {
               ...
               game.movement = true;
               await goatbro.walk(renderer, 3, { y: 30 });
            }
         }
         break;
      }
