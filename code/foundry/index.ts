...
export const foundryScript = async (subscript: string, ...args: string[]): Promise<any> => {
   ...
   if (subscript === 'tick') {
      if (!foundryStates.scripts.steamgap || !foundryStates.scripts.steamgap.init) {
         ...
         await renderer.pause(CosmosMath.remap(Math.random(), 500, 5000));
         if (renderer.alpha.value === 1 && !battler.active) {
            ...
            if (steamgap && steamgap.length > 0) {
               ...
               sprite.scale.modulate(renderer, 500, sprite.scale.multiply(CosmosMath.remap(Math.random(), 1.1, 1.3)));
            ...
            if (!roomState.active) {
               ...
               if (SAVE.data.n.plot < 37.11) {
                  ...
                  if (world.genocide) {
                     ...
                     kiddo.face = 'right';
                     await Promise.all([ kiddLoader, renderer.pause(850) ]);
         ...
         case 'f_entrance': {
            ...
            if (!roomState.meetMK) {
               ...
               if (world.genocide && SAVE.data.n.plot < 38.01) {
                  ...
                  await Promise.all([ kiddLoader, renderer.pause(1250) ]);
      ...
      switch (subscript) {
         ...
         case 'spookydate': {
            ...
            if (world.edgy_x) {
               ...
               if (choicer.result === 0) {
                  ...
                  await Promise.all([ whoopeeLoader, player.walk(renderer, 3, { x: 187.5, y: 127 }) ]);
                  await player.walk(renderer, 1.5, { y: 106.5 });
                  const whooper = sounds.whoopee.instance(renderer);
                  whooper.rate.value = 1.25;
                  await Promise.all([ grillbyLoader, renderer.pause(2500) ]);
                  ...
                  await Promise.all([ renderer.pause(850), premonitionLoader ]);
                  ...
                  await Promise.all([ elevatorMachineLoader, renderer.pause(350) ]);
                  mode = 4;
                  await renderer.pause(350);
         ...
         case 'birdcheck': {
            ...
            if (choicer.result === 0) {
               ...
               await Promise.all([ riseLoader, renderer.pause(500) ]);
            ...
            if (game.movement) {
               ...
               if (postSIGMA()) {
                  ...
                  if (choicer.result < 3) {
                     ...
                     await Promise.all([ thing.load(az), dialogue('auto', ...text.a_foundry.blookmusic1y) ]);
            ...
            if (game.movement && player.face === 'up') {
               ...
               if (roomState.blookie && SAVE.data.n.state_foundry_blookdate === 1) {
                  ...
                  if (choicer.result === 0) {
                     ...
                     await Promise.all([ chillLoader, renderer.pause(1250) ]);
                                 ...
                                 switch (SAVE.data.n.state_foundry_swansong) {
                                    ...
                                    case 2:
                                       ...
                                       if ((choicer.result as number) === 0) {
                                          ...
                                          await Promise.all([ secretLoader, renderer.pause(1450) ]);
                                          renderer.pause(850).then(() => {
                  ...
                  case 'f_snail':
                     ...
                     if (SAVE.data.n.state_foundry_blookdate === 2) {
                        ...
                        if (choicer.result === 0) {
                           ...
                           await Promise.all([ snailLoader, renderer.pause(500) ]);
                           ...
                           await Promise.all([ thunderLoader, renderer.pause(1000) ]);
         ...
         case 'undyneboss': {
            ...
            await Promise.all([ renderer.pause(1000), fishAssets.load() ]);
            ...
            await Promise.all([ renderer.pause(1000), moosicLoader ]);
            if (papyrusKiller) {
               undyne.face = 'right';
               await renderer.pause(850);
         ...
         case 'undynefight': {
            ...
            const eggoAssets = new CosmosInventory(inventories.iocUndyne, content.asStomp, content.amUndynefast);
   ...
   tem: new OutertaleShop({
      ...
      purchase (buy) {
         ...
         if (success) {
            ...
            shops.tem.vars.purchase = 0;
            const inst = music.temShop.instances[0];
...
export async function foundryTPE (from: string, to: string) {
   ...
   if (SAVE.data.n.epiphany === 1 && !SAVE.data.b.svr && !world.runaway) {
   ...
   switch (to) {
      case 'f_truth': {
         (SAVE.data.n.epiphany === 2 || SAVE.data.b.svr || world.runaway) && (instance('main', 'tomebox')!.index = 1);
         break;
      }
      case 'f_view': {
         const scale = 0.5;
      ...
      case 'f_story1': {
         if (!roomState.active) {
            ...
            if (world.genocide && SAVE.data.n.plot < 37.1) {
               ...
               game.music?.stop();
               player.face = 'up';
               await Promise.all([ asgoreLoader, renderer.pause(850) ]);
         ...
         if (SAVE.data.n.plot < 46 && !world.genocide) {
            ...
            if (SAVE.data.n.state_foundry_muffet === 1) {
               ...
               if (res === 0) {
                  ...
                  await Promise.all([ renderer.pause(650), sfxLoader ]);
                  fish.preset = characters.undyneArmorJetpack;
                  sounds.noise.instance(renderer);
                  const flame = sounds.jetpack.instance(renderer);
                  flame.gain.value *= 0.4;
      ...
      case 'f_plank': {
         if (SAVE.data.n.plot < 42) {
            ...
            await Promise.all([ memoryLoader, renderer.pause(3000) ]);
            game.movement = false;
      ...
      case 'f_dummy': {
         if (SAVE.data.n.plot < 42.1) {
            ...
            cam.position.x = 160;
            await Promise.all([ bigLoader, renderer.pause(850) ]);
      ...
      case 'f_battle': {
         ...
         if (world.genocide && SAVE.data.n.plot < 48) {
            ...
            await Promise.all([ fishLoader, renderer.pause(UF === 0 ? 850 : 350) ]);
            ...
            const cym = sounds.cymbal.instance(renderer, { offset: UF === 0 ? 2.5 : 3.5 });
            cym.gain.value /= 4;
      ...
      case 'f_kitchen': {
         ...
         await Promise.all([ moosicLoader, renderer.pause(1750) ]);
...
events.on('use', async (key, index) => {
   switch (key) {
      case 'artifact':
         if (game.room === 'f_truth' && SAVE.data.n.epiphany < 1 && !SAVE.data.b.svr && !world.runaway) {
