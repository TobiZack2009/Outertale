...
import { battler, dialogue_primitive, shake, world } from '../systems/framework';
      ...
      init () {
         ...
         if (world.genocide) {
            ...
            events.on('battle').then(async () => {
               ...
               await Promise.all([ pregenoLoader, renderer.pause(UF === 0 ? 1250 : 850) ]);
               ...
               battler.music = (() => {
                  ...
                  inst.source.addEventListener('ended', () => {
                     if (battler.music) {
                        const newinst = music.undynegeno.instance(renderer, { offset: 21.81 });
