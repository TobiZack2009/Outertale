...
export const friskChara = {
   down: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskDownChara }),
   left: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskLeftChara }),
   right: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskRightChara }),
   up: new CosmosAnimation({ anchor: { x: 0, y: 1 }, resources: content.iocFriskUpChara })
};
...
export const basic = new CosmosObject({
   ...
   let index = 0;
   const par = [ 0, 1, 2, 3, 4, 3, 2, 1 ];
   const collections = [ [], [], [], [], [] ] as number[][][];
   while (index < 120) {
      collections[par[Math.floor(((time + index) % 40) / 5)]].push(
               ...
               ob.on('tick', function () {
                  if (typer.mode !== 'empty' && speech.state.preset === pr) {
                     switch (ultimaFacer({ position: ob, size: (ob.objects[1] as CosmosHitbox).size })) {
                        case 'down':
                           an.resources = content.iocFriskDownChara;
                           break;
                        case 'left':
                           an.resources = content.iocFriskLeftChara;
                           break;
                        case 'right':
                           an.resources = content.iocFriskRightChara;
                           break;
                        case 'up':
                           an.resources = content.iocFriskUpChara;
                           break;
                     }
                  } else {
                     an.resources = re;
   ...
   switch (to) {
      ...
      case '_cockpit': {
         ...
         if (!world.genocide) {
            ...
            if (SAVE.flag.n.lv20 < 4) {
               ...
               await Promise.all([ splashLoader, renderer.pause(2000) ]);
               saver.save('_cockpit');
               SAVE.flag.n.lv20 = 4;
      ...
      case '_frontier8': {
         ...
         if (SAVE.isCanon() && SAVE.data.b.svr) {
            ...
            newworld.stop();
            realAssets.unload();
            await Promise.all([ endLoader, renderer.pause(3000) ]);
         ...
         SAVE.isCanon() && saver.svr && !SAVE.data.b.svr && (SAVE.flag.b.true_reset_spiel = true);
         SAVE.flag.b.true_reset = true;
         ...
         renderer.attach(
            ...
            new CosmosText({
               fill: 0xffffff,
               fontFamily: content.fDeterminationSans,
               content: saver.svr ? systemsText.extra.end1 : systemsText.extra.end2,
               anchor: { x: 0 },
