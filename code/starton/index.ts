   ...
   if (subscript === 'tick') {
      switch (game.room) {
         case 's_start':
            ...
            if (world.genocide) {
               ...
               await Promise.all([ renderer.pause(550), shockLoader ]);
               jeebs.stop();
               ...
               await Promise.all([ stuffLoader, renderer.pause(750) ]);
               ...
               await battler.simple(async () => {
                  ...
                  const cym = sounds.cymbal.instance(renderer, { offset: 1.5 });
                  cym.gain.value /= 4;
            ...
            await Promise.all([ renderer.pause(5750), muscleLoader ]);
            ...
            if (!world.edgy) {
               ...
               await Promise.all([ papyrusLoader, renderer.pause(850) ]);
            ...
            if (!roomState.active) {
               ...
               if (SAVE.data.n.plot < 28) {
                  ...
                  if (SAVE.data.n.state_starton_greatdog < 1) {
                     await Promise.all([ lickLoader, renderer.pause(450) ]);
            ...
            if (!roomState.active) {
               ...
               if (SAVE.data.n.plot < 30) {
                  if (world.genocide) {
                     ...
                     await Promise.all([ renderer.pause(1000), impactLoader ]);
      ...
      switch (subscript) {
         ...
         case 'trashhunt': {
            ...
            await Promise.all([ sansbossLoader, renderer.pause(1000) ]);
         ...
         case 'xtower': {
            ...
            const endGame = async () => {
               ...
               stopanim = true;
               const newScore = Math.min(score, 1000000);
            ...
            while (!ended) {
               spawnTarget();
               await renderer.pause(CosmosMath.bezier(Math.min(renderer.time - time, 15e4) / 15e4, 2500, 600));
            }
            break;
         }
         case 'puzzlechip': {
            ...
            if (game.movement) {
               ...
               if (postSIGMA()) {
                  ...
                  if (choicer.result < 3) {
                     ...
                     await Promise.all([ thing.load(az), dialogue('auto', ...text.a_starton.jukebox1y) ]);
                     barmusic(roomState);
                     const precount = [
         ...
         case 'papdate': {
            ...
            if (game.room === 's_bonehouse') {
               ...
               if ((choicer.result as number) === 0) {
                  ...
                  keys.menuKey.on('down', hudMenuListener);
                  await Promise.all([ readLoader, renderer.pause(350) ]);
                  keys.menuKey.off('down', hudMenuListener);
      ...
      case 's_maze': {
         ...
         if (SAVE.data.n.plot < 20) {
            ...
            if (world.genocide) {
               ...
               await renderer.pause(450);
               goatbro.face = 'right';
               await Promise.all([ notifier(paps, true, 45), papyrusLoader ]);
               paps.face = 'left';
               const papsMusic = music.papyrus.instance(renderer);
               papsMusic.rate.value = 0.325;
               await renderer.pause(1250);
               ...
               sand.face = 'left';
               paps.face = 'left';
               await Promise.all([ renderer.pause(850), papyrusLoader ]);
      ...
      case 's_bros': {
         if (SAVE.data.n.plot < 21) {
            ...
            if (world.genocide) {
               ...
               game.camera = cam;
               await Promise.all([ cam.position.modulate(renderer, 1000, { x: 360 }), muscleLoader ]);
               const papsMusic = music.papyrus.instance(renderer);
               papsMusic.rate.value = 0.325;
               await renderer.pause(650);
               ...
               await Promise.all([ renderer.pause(650), muscleLoader ]);
      ...
      case 's_jenga': {
         ...
         if (SAVE.data.n.plot < 26) {
            const thePuzzle = async (paps: CosmosCharacter, papsMusic: CosmosInstance, xdiag = true) => {
               ...
               await Promise.all([ renderer.pause(650), compooterLoader ]);
               sounds.equip.instance(renderer).rate.value = 1.25;
            ...
            if (world.edgy) {
               ...
               if (world.genocide) {
                  goatbro.face = 'right';
                  await Promise.all([ dialogue('auto', ...text.a_starton.genotext.papyrusSolo3), papyrusLoader ]);
                  const papsMusic = music.papyrus.instance(renderer);
                  papsMusic.rate.value = 0.325;
                  await renderer.pause(850);
                  header('x1').then(() => {
                  ...
                  await Promise.all([ renderer.pause(1250), papyrusLoader ]);
                  paps.face = 'left';
